...
import pandas as pd
import numpy as np
from utils.munging_utils import get_file_entity, save_data_to_synapse 
from utils.preprocessing_utils import preprocess
...
def main():
    ...
    dataV1 = get_file_entity(WALKING_V1)
    ...
    dataV2 = get_file_entity(WALKING_V2)
    ...
    dataPassive         = get_file_entity(WALKING_PASSIVE)
    ### Ensure that per healthcodes is not only contributing one data (>5 recordIds threshold) ###
    dataPassive         = preprocess(dataPassive, "max", True)
